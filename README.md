...
O pipeline processa documentos heterogêneos, podendo utilizar extração de texto direta ou OCR, classifica o evento corporativo, extrai os dados relevantes e realiza validações contra os registros de referência e contra regras de coerência.
...
O LLM é responsável principalmente pela interpretação do conteúdo e classificação do evento, enquanto as etapas determinísticas de extração, validação e comparação com os registros de referência são controladas pelo código Python.